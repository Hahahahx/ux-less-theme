 ...
 */
// @ts-ignore
function shake(isShaking, ruleset, getVars, vars, map) {
    /**
     * 递归遍历语法树
     * @param rule
    ...
    ruleset.forEach((rule, index) => {
        try {
            const event = new type_event_1.TypeEvent(rule)
                ...
                if (vars && args) {
                    const params = rule.params.filter((item, index) => {
                        return args[index];
                    });
                    vars = [
                        ...vars,
                ...
                if (vars) {
                    ...
                    let args = rule.arguments.map((item) => {
                        return (item &&
                            utils_1.isType(item.value, "Expression") &&
                            utils_1.isExpression(item, vars));
                    });
                    ...
                    if (args.includes(true)) {
                        ...
                        if (params) {
                            args = params.map((item, index) => item || args[index]);
                        }
