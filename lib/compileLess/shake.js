    ...
    ruleset.forEach((rule, index) => {
        try {
            const event = new type_event_1.TypeEvent(rule)
                ...
                let params = rule.params.map((item) => item.name);
                ...
                rule.rules.forEach((item) => {
                    if (item.variable) {
                        ...
                        if (Array.isArray(item.value.value)) {
                            if (utils_1.isExpression(item, params)) {
                                getVars(item);
                            }
                        }
                    }
                    else {
                        utils_1.isExpression(item, params) &&
                            resultsDefinition.push(item);
                    }
                });
                ...
                rule.arguments.some((item) => {
                    if (utils_1.isExpression(item, vars)) {
                        results.push(rule);
                        return true;
                    }
                });
