...
const filterVars_1 = require("./filterVars");
const TypeEvent_1 = require("./TypeEvent");
...
function shake(ruleset, getVars, vars) {
    ...
    ruleset.forEach((rule, index) => {
        try {
            const event = new TypeEvent_1.TypeEvent(rule)
                .setImport(() => {
...
function shaking(rules, modifyVars) {
    ...
    // 判断依赖
    const newVars = filterVars_1.filterVars(Object.values(allVars), vars);
